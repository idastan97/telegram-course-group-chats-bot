var tg = require('./telegram-cli/index.js')('/opt/tg/bin/telegram-cli', '/opt/tg/tg-server.pub');

tg.on('ready', function () {

    tg.sendRAW('dialog_list');
    ...
    tg.on('message', function (message) {
        ...
        if (args[0] === "/REG") {
            var j;
            var found = false;
            for (j = 0; j < courses.length && !found; j++) {
                var abbrs = courses[j].abbr.split('/');
                var k;
                for (k = 0; k < abbrs.length; k++) {
                    if (args[1] === abbrs[k]) {
                        var outstr = "chat_add_user chat#" + courses[j].id + " user#" + message.name;
                        tg.sendRAW(outstr);
                        found = true;
                        console.log('added');
                    }
